...
(comment
      ...
      (when-let [value (<! ops|)]
        (condp = (:op value)
          ...
          (let [{:keys [frequency role]} value
                ...
                games-topic (Jar-Jar.corn/encode-base64url-u "raisins")
                game-topic (Jar-Jar.corn/encode-base64url-u frequency)
                _ (Jar-Jar.corn/subscribe-process
                   {:sub| sub|
                    :cancel| cancel-sub|
                    :frequency frequency
          ...
          (let [discover-jframe (JFrame. "discover")]
            (Jar-Jar.kiwis/discover-process
          ...
          (let [settings-jframe (JFrame. "settings")]
            (Jar-Jar.kiwis/settings-process
  ...
  (let [port (or (System/getenv "Jar_Jar_IPFS_PORT") "5001")
    ...
    (Jar-Jar.corn/subscribe-process
     {:sub| sub|
